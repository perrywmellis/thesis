...
\begin{figure}
  ...
  The contours in (B) have (black squares) $\Gamma = 7.3$, (red circles) $\Gamma = 4.6$, (blue triangles) $\Gamma = 2.0$, (magenta downward triangles) $\Gamma = 1.0$, (cyan diamonds) $\Gamma = 0.6$, and (green stars) $\Gamma = 0.4$.
  The contours in (D) have (black squares) $\Gamma = 4.5$, (red circles) $\Gamma = 3.9$, (blue triangles) $\Gamma = 1.9$, (magenta downward triangles) $\Gamma = 1.5$, and (cyan diamonds) $\Gamma = 1.3$.}\label{f:5-ShapeContour}
\end{figure}
...
\begin{figure}
  ...
  Starting at a large $\Gamma$ in the ring-defect state (open circles) and decreasing $\Gamma$ leads to a transition to a point defect (x symbols) at a value of $\Gamma = 2.7 \pm 0.3$, which we obtain by averaging the result for all bridges.
  ...
  The squares are experimental measurements; the circles correspond to computations in a waist structure using the elastic constants for 5CB.\@
...
\begin{figure}
  ...
  Note how the sodium dodecyl sulfate (SDS) used to enforce homeotropic anchoring in the barrels forms micelles and disrupts the director in the bridge over time, from the initial measurement in (A,B) to the final measurement pictured in (G,H).}\label{f:5-ExpBarrelTop}
\end{figure}
...
\begin{figure}
  ...
  We label the experimental points with their measurement number, with black squares corresponding to the first measurement, red squares the second measurement, blue squares the third measurement, and magenta squares the fourth measurement, where we start at a large $\Gamma$ and decrease $\Gamma$ with each subsequent measurement in a single barrel.
  The circles correspond to computations in a barrel structure using the elastic constants for 5CB.\@
  ...
  Here, the defect structure is a point and is plotted in (A) as such.}\label{f:5-ExpBarrelRing}
\end{figure}
...
Polarized fluoresence has proven to be an incredibly effective tool in diagnostic imaging and the medical community, from techniques like F\"oster Resonance Energy Transfer (FRET), to the myriad of assays designed to measure quantities such as protease content, kinase content, and even cellular or molecular mobility~\cite{RN282,RN284}.
...
\begin{figure}
  ...
  (K), The $\delta'$ from a PFM of every pixel in (G-I) plotted on top of an epifluorescent image of the droplets.
  The droplets are clearly radial, as expected from the PA rotation in (B,C).}\label{f:5-PFM_Spheres}
\end{figure}
