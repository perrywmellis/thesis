...
We now have a system of three equations to fit with 5 parameters, as $L^2_1 = L^1_2$.
If we naively expand Eq.~\ref{e:3-Kfit2} directly into three equations,
\refstepcounter{equation}\label{e:3-KfitNaive}
\begin{align}
  \Delta k_1 &= L^1_1 \Delta R_1 + L^2_1 \Delta R_2\tag{\theequation a} \\
  \Delta k_2 &= L^1_2 \Delta R_1 + L^2_2 \Delta R_2\tag{\theequation b} \\
  \Delta k_k &= M_1 \Delta R_1 + M_2 \Delta R_2,\tag{\theequation c}
\end{align}
where we have expanded the vectors like $\mathbf{v}\cdot \mathbf{e}_i = v^j g_{ij} = v_i$ and $\Delta k_k$ refers to the component of $\Delta \mathbf{k}$ along the surface normal, we see that $L^2_1$ will be determined entirely by the variation along $\mathbf{e}_2$.
The result of $L^2_1$ from fitting Eq.~\ref{e:3-KfitNaive}a will then set the value of $L^1_2$ in the fit of Eq.~\ref{e:3-KfitNaive}b so that only $L^2_2$ is free to vary during the fit.
...
\begin{align}
  \Delta k_2 \Delta R_2 - \Delta k_1 \Delta R_1 &= (\Delta R_2)^2 L^2_2 - (\Delta R_1)^2 L^1_1\tag{\theequation a}\\
  \Delta k_1 + \Delta k_2 - L^1_1 \Delta R_1 - L^2_2 \Delta R_2 &= (\Delta R_1 + \Delta R_2)L^2_1\tag{\theequation b}\\
  \Delta k_k &= M_1 \Delta R_1 + M_2 \Delta R_2.\tag{\theequation c}
\end{align}
If we do the fits in order, the values of $L^1_1$ and $L^2_2$ from fitting Eq.~\ref{e:3-KfitConstrained}a are inserted as fixed parameters into the fit of Eq.~\ref{e:3-KfitConstrained}b.
However, now the value of $L^2_1$ is determined by the variation in both directions in the tangent plane.
...
We use the fitting weights from the final iteration $(p)$ and the final values in $\bm{\Lambda}$ to correct our initial estimate of $\mathbf{k}_0$ according to:
\begin{equation}
\mathbf{k}_0^{(p)} = \frac{\sum_i m_{0i}w_{0i}^{(p)}(N_i^{(0)} - \mathbf{\Lambda}^{(p)}\mathbf{\Delta R}_{0i})}{\sum_j m_{0j}w_{0j}^{(p)}},.
\end{equation}
...
As a consequence, we typically neglect anywhere from 10~px to 30~px from the boundary of the final upsampled $K(x,y)$.
...
We start by considering a binary mask $\Theta(x,y)$ where the region in the intensity projection where we have data for the director field has a value $1$ and everywhere else has a value $0$.
...
For any mask defining a region, we calculate the integrated Gaussian curvature with the sum $\sum_{x,y} \Theta(x,y) K(x,y) \sqrt{g}(x,y)$.
