...
\end{figure}\fxnote{Check actual pluses and minuses}
...
We see in this example that 76 frames out of 262 frames have an error; however, the error is typically only $\pm 1/2$, with only three frames having an error of $\pm 1$\fxnote{Check actual pluses and minuses}.
Importantly, we see that ${\rm mean}(error) = 0.026$ when we consider all the frames.
Since the mean error is much less than $1/2$, we see that the error in our analysis is random and we can trust the time-averaged topological charge.
...
In addition, this result is consistent with the fact that when the ATP runs out in an experiment and the activity ceases, the nematic configuration becomes static and does not relax to the ground state; the elasticity in the experiments is too weak\fxnote{rethink about this} when $\alpha = 0$.
