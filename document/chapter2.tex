...
\end{figure}\fxnote{tilt the particle we measure off of, define the axes e1, e2}
...
We need $\mathbf{Q}$ to be traceless if we wish to use $\mathbf{Q}$ as an order parameter with $Q_{ij} = 0$ in the isotropic phase.\fxnote{more concerning an order parameter and the need to go to 0}
...
In fact, it is easy to see that a mapping of the director along the blue contour in Figure~\ref{f:2-2DMeas}(C) will result in a contour in $\mathbb{R}\mathbb{P}^1$ that spans $\mathbb{RP}^1$ and goes in opposite direction to the red contour in  Figure~\ref{f:2-2DMeas}(B).
...
\end{equation}
where $\partial A$ is the boundary of some area $A$ containing the defect and the integral is performed along the boundary.\fxnote{ask alberto about his note "wrt the area to the left!"}
...
\end{figure}\fxnote{split (B) into 2 diagrams. draw the contours ON the space}
...
Again, we consider $\mathbf{n}$ along a closed 1D contour in real space and the associated 1D contour in $\mathbb{R}\mathbb{P}^2$, with the topological charge given by the half-integer winding number of the associated contour in $\mathbb{S}^2$.\fxnote{check this and be careful.  come back to the paragraph after thinking about it and rereading}
...
Thus, the only stable contours in $\mathbb{R}\mathbb{P}^2$ are those that start and end at antipodal points, wrapping $\mathbb{R}\mathbb{P}^2$ exactly once.\fxnote{need a figure. BALLS}
...
We lastly consider point defects in 3D characterized by the second fundamental group of $\mathbb{R}\mathbb{P}^2$, $\pi_{2}(\mathbb{R}\mathbb{P}^2)$.
Now, instead of mapping 1D contours on $\mathbb{R}\mathbb{P}^2$ to $\mathbb{S}^1$ as we did with the first fundamental group, we consider 2D coverings of $\mathbb{R}\mathbb{P}^2$ mapped onto $\mathbb{S}^2$~\cite{RN196,RN153,RN236}.\fxnote{check this again.}
...
For example, consider projecting a hemisphere onto the Earth's globe.\fxnote{this is a bad example.}
...
The splay-bend term itself is subject to controversy as its inclusion to the free energy makes the free energy impossible to minimize~\cite{RN214,RN215,RN216}.\fxnote{go back and do this better, see notes}
...
Since the interaction potential is positive definite, we see from the microscopics that any elastic constant in Eq~\ref{e:2-FrankFinalExpansion} or Eq.~\ref{e:2-NSFinalExpansion} must be greater than or equal to 0~\cite{RN205}.\fxnote{why do people talk about K24<0, see lavrentovich}
...
Since there are an infinite number of possibilities to perform the nonlocal-to-local mapping $(\mathbf{R},\mathbf{R}') \rightarrow \mathbf{r}$, any elastic constants in $f(\mathbf{r})$ cannot depend on the choice of mapping~\cite{RN55}.\fxnote{illustrate this better, see notes.}
...
For a distortion of wavelength $\xi$ and amplitude $\epsilon$, we can only uniquely determine $\mathbf{n}(\mathbf{r})$ if we restrict ourselves to the lowest energy distortions in the the long-wavelength $(\xi \rightarrow \infty)$ and small amplitude $(\epsilon \ll 1)$ regime. \fxnote{see notes do this better}
...
In addition, since the free energy is constructed using a positive-definite pairwise potential, all the elastic constants in the phenomenological free energy expressions must be greater than or equal to 0.\fxnote{see Lavrentovich ... moving on}
...
Lyotropic LC's are suspensions of particles in a solvent and therefore are sensitive to both concentration as well as to temperature.\fxnote{unreadable Alberto comment}
...
This technique takes advantage of the birefringence of the NLC to determine\fxnote{why is this circled?} the director in the sample.
...
\end{figure}\fxnote{fix the ordering}
...
\end{table}\fxnote{not at all done}
