...
Again, we consider $\mathbf{n}$ along a closed 1D contour in real space and the associated 1D contour in $\mathbb{R}\mathbb{P}^2$, with the topological charge given by the half-integer winding number of the associated contour in $\mathbb{S}^1$.
...
Recall from Eq.~\ref{e:1-TopTheoryofDefects} that the free energy of a nematic configuration with defects on a surface has the exact form of the free energy of a multicomponent plasma, where the defects act like the charges in the plasma.
...
Since the constituent particles, or mesogens, in a nematic material prefer to align along the $\mathbf{n}$, the ideal state for a nematic phase is a homogeneously-aligned domain with $\mathbf{n}$ a constant everywhere.
Distortions from this uniform state cost energy.
Since in most experiments the director distortions occur over much larger length scales than the length of the mesogens --- $|\nabla \mathbf{n}| a \ll 1$, where $a$ is the mesogen length --- we can forget about the behavior of the individual mesogens and instead use a continuum model for the free energy density that depends on $\mathbf{n}$ only.
...
Let a local coordinate system at a point be defined by $\{x_1, x_2, x_3 \}$ such that we can define $\mathbf{n} = (\mathbf{n} \cdot \hat{e}_i) \hat{e}_i = n_i$, where we again sum over repeated indices, and $\hat{e}_i$ is the unit vector associated with $x_i$.
If we let $\hat{e}_3$ be parallel to $\mathbf{n}$ at the point and $x_i \ll 1$, we can locally expand $n_i$ as:
\begin{align}
...
However, any free energy must respect the symmetry of the nematic, hence it must be invariant under exchange of $i$ and $j$, invariant under inversion of $\mathbf{n} \rightarrow -\mathbf{n}$, invariant under arbitrary rotations about $\mathbf{n}$, and invariant with respect to the handedness of the coordinate system~\cite{RN61}.
...
Finally, we can move from the local frame and recast the $a_i$ in Eq.~\ref{e:2-FrankLocalExpansion} in terms of a single global coordinate system, arriving at the well-known expression for the Frank-Oseen Free Energy~\cite{RN61}:
\begin{align}
...
\end{equation}
Now that we again have a total differential, we can use the divergence theorem and write saddle-splay free energy in Eq.~\ref{e:2-positionF24} as:
\begin{equation}
...
\end{equation}
Since $\mathbf{k} \cdot \mathbf{n} = 0$, we can re-write the right-hand side (RHS) of Eq.~\ref{e:2-K24rearrange1} as:
\begin{equation}
...
\end{equation}
where $\mathbf{L} = blah$\fxnote{HERE} is the Weingarten Matrix~\cite{RN23,RN35}.

The Weingarten Matrix ... borrow from appendix and introduce ideas of curvature.
...
This lets us write the saddle-splay free energy in terms of the principal curvatures $\kappa_1$ and $\kappa_2$ as~\cite{RN59}:
\begin{equation}
  F_{24} = \frac{1}{2}(K_{22} + K_{24})
  \oint_{\partial V}\textup{d}^2 \mathbf{r} \left \{\kappa_1 n_1^2 + \kappa_2 n_2^2 \right \},\label{e:2-K24SurfCouple}
\end{equation}
where $n_1$ and $n_2$ are the components of the director along the $1^{\rm st}$ and $2^{\rm nd}$ principal curvatures, respectively.
...
\end{align}
Now we can re-write Eqs.~\ref{e:2-LdGDistGeneralQuadOrder} using Eqs.~\ref{e:2-LdGRelationsQuadOrderA}--\ref{e:2-LdGRelationsQuadOrderC} and collect terms like:
\begin{align}
...
This phase difference is known as the retardation and is given in multiples of $2 \pi$ by $\Gamma = 2 \pi \Delta n \, d / \lambda$, where $d$ is the thickness of the birefringent material, and $\lambda$ is the wavelength of the incident light~\cite{RN232}.
