...
This is illustrated by the red arrows in Figure~\ref{f:3-ActiveInteraction}(B,D), and thus we see that extensile interactions have $\alpha < 0$ while contractile interactions have $\alpha > 0$~\cite{RN11}.
...
While low activity can suppress the spontaneous formation of defect pairs in an active nematic~\cite{RN247,RN7}, the steady-state of an active nematic is often full of pairs of $s = \pm 1/2$ defects that are continuously created and annihilated~\cite{RN11,RN8,RN3,RN27,RN135,RN86}.
...
Equating the two timescales and rearranging terms allows us to express this condition as: $\textup{Ca}_o > (2 \pi / f(\mu_i/\mu_o))R_{tip}/a_{jet}$, where $\textup{Ca}_o = \mu_o U/\gamma$ is a dimensionless group called the Capillary number quantifying the relative importance of the viscous and surface tension stresses.
...
To achieve this, we need to counter the surface tension stress responsible for destabilizing a toroidal droplet.
...
The first minimum of this pattern has a diameter~\fxnote{REF}:
\begin{equation}
...
This is known as the Rayleigh criterion; Eq.~\ref{e:3-Airy} thus also gives the resolution for an objective with a given NA~\fxnote{REF}.
...
The best resolution occurs when the pixel length is $d/2$; however, this means that the total sample area imaged is small.
Typically, we choose a pixel length larger than $d$, to sacrificing resolution but increasing the area of the sample we image.
...
In this regime, the resolution is determined by the full-width half-maximum (FWHM) of the intensity distribution in the axial direction~\fxnote{cite Zeiss microscopy prinicples}:
\begin{equation}
...
However, even in scanning laser confocal microcopy, a high enough laser intensity or a long enough imaging time will always result in significant photobleaching in the sample.

In our experiments, only the microtubules are labeled with fluorophores, such that we only image the active filaments.
Thus, as shown in the example image in Figure~\ref{f:3-Confocal}(C), where the height is denoted by false color, we can acquire a 3D image stack showing only the active nematic depleted to the toroidal surface.\fxnote{still need example NA for our objective and typical resolutions in xyz}
\begin{figure}
  \centering
...
\end{figure}\fxnote{need typical scan areas and numbers}
...
\end{figure}\fxnote{switch panels}
...
Each 512 px $\times$ 512 px image plane typically takes blah s to acquire, and it takes about blah to move between planes.
Overall, this means that we aim to acquire a full image stack every blah s~\fxnote{specifics please}.
...
The lasers are a blah that is adjusted via blah.
The gain on the detector is a blah, also controlled in the blah\fxnote{what actually tunes the laser intensity and the gain}
...
Once these parameters have been set, we turn on a focusing system to eliminate the effect of thermal and mechanical drift in the measuring system.\fxnote{how does perfect focus work}
...
There are a large range of threshold values that do not change the shape of the surface or affect the final curvature values; we choose the smallest threshold value that produces a smooth-looking surface with no extraneous bright points in remainder of the image stack.
...
We track $s = +1/2$ and $s = -1/2$ defects independently using a combinatorics-based particle tracking algorithm~\cite{RN54}.\fxnote{reference note on the algorithm}
...
We do this by placing the defect locations at each time frame into a binary array, or mask, where a ``1'' indicates a defect and the rest of the array set to ``0'.'
...
We characterize the error in the measured topological charge by considering a region of interrogation as its own entity, i.e. a compact surface with a boundary.
...
This moving frame is called the Frenet-Serret frame and is defined by the unit tangent and unit normal vectors along the boundary.\fxnote{need to define the Frenet-Serret frame in a figure, do EdgeChargeEx better.}
...
This is illustrated with examples using a circular region containing a defect-free director field, a director field with a single $s = +1$ defect, and a director field with a single $s=-1/2$ defect in Figure~\ref{f:3-EdgeChargeEx}(A-C), respectively.
...
\end{figure}\fxnote{this figure is not clear. do better and illustrate FS frame}
...
Since each missed defect introduces an error of $\pm 0.5$, the negligible average error indicates that there is no systematic error in our analysis.
When we do this for multiple toroids, we find that in general there is $\approx 50$\% chance that a measurement will have an error, but the error is random and not systematic such that $\overbar{s}_{bulk} + \overbar{s}_{boundary} = 1$ if we average for enough frames.\fxnote{explain the calculation a bit better in terms of the number of defects in a region}
...
Thus, we have three material parameters, $\alpha$, $\eta$, and $k_F$ from a continuum description of active nematics that determine how a collection of active defects behave.
