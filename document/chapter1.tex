...
For example, the particle collection can transition from a fluid phase to a crystalline phase, where the particle positions make up a periodic lattice.
Thus, the system is now only invariant with respect to a discrete set of translations and rotations.
In this example, the system breaks the continuous symmetry of the isotropic phase in order to achieve the crystalline phase.
...
For example, ordered materials often exhibit birefringence, where the index of refraction experienced by a light ray incident on the material depends on the direction and polarization of the light ray~\cite{RN175}.
...
In a crystalline material, these grain boundaries affect the moduli of the material, and are even responsible for the phenomenon of ``work hardening,'' where plastically deforming (``working'') the material strengthens the shear modulus.
...
First, pairs of rotational defects, or disclinations~\cite{RN61,RN203}, proliferate, driving the crystalline phase into an intermediate hexatic phase.
Second, these disclination pairs unbind, transforming the hexatic phase into the isotropic phase.
Furthermore, defects in soft matter have been used as tools to investigate a wide range of phenomena from knot theory~\cite{RN156}\fxnote{experimental knots} to controlled self-assembly~\cite{RN43,RN50,RN150,RN157}, to hierarchical materials~\cite{RN164,RN159,RN27}.
...
We can characterize a surface with the Gauss-Bonnet theorem; here writing the theorem for a closed surface~\cite{RN23}:
\begin{equation}
...
Since a sphere has no handles and thus $\chi=2$, Eq.~\ref{e:1-GB1} implies that any surface with $g=0$ is topologically equivalent, or homoemorphic, to the sphere and thus also has $\chi=2$.
...
Short of moving the disclination through $\partial A$, we cannot eliminate the enclosed disclination with smooth deformations of the director; thus, the disclinations are topological and $s$ is known as the ``topological charge.''
...
\end{figure}\fxnote{add a thing showing the difference between nematic and polar}

Since the individual particles in such an orientationally ordered phase prefer to align parallel to each other, we can express the distortion free energy in the continuum limit as a functional of the orientation field,
\begin{equation}
...
However, this configuration only exists in thin enough shells, as characterized by the relative shell thickness $h' = (R^{out}-R^{in})/R^{out}$, with $R^{in}$ and $R^{out}$ the inner and outer radius of the nematic shell, as defined schematically in Figure~\ref{f:1-Shells}(C).
...
In fact, in the cylindrical limit where $\xi \rightarrow \infty$, the free energy of the doubly-twisted state has exactly the functional form of the Landau theory of magnetism~\cite{RN191}\fxnote{cite our paper}.
...
\end{figure}
Up to this point we have considered how constraints imposed on a surface affect ordered materials that lie on the surface or are confined by the surface.
...
Geometrically, the hedgehog charge relates the orientations of $\mathbf{n}$ taken on topologically spherical surface enclosing the defect to the number of times the orientations of $\mathbf{n}$ cover the unit sphere~\cite{RN153}.\fxnote{do this better}
