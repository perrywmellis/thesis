...
To make a gel, we have to construct a sample chamber where the depletion interaction cannot drive the bundles to aggregate at an interface.
We build the sample chamber from a glass coverslip attached via two-sided tape to a microscope slide to form a rectangular channel.
Prior to construction, we coat the coverslip and slide with polyacrylamide brushes such that the PEG can penetrate between the brushes, keeping the depletion interaction from driving the active filaments to the interface, where they would asorb to the glass~\cite{RN3}.
...
To make the rectangular sample chambers, cut strips of 2-sided tape such that the are $\approx1$ mm wide and longer than the coverslip.
...
We generate our toroidal droplets following the procedure detailed in Refs.~\cite{RN29,RN47}\fxnote{Teaching RP}.
...
We control the position of the needle in the cuvette with a micromanipular and we control the flow rate and volume of the inner phase with a syringe pump.
In addition, we place a camera below the roation stage such that we can image the droplet generation process.
...
Surface tension acts to minimize the interfacial area between two immisicble fluid phases, hence the ubiquity and stability of spherical droplets in nature; the sphere minimizes the surface area for a given volume.
The physics behind the surface tension stress at an interface comes through the pressure drop across a fluid-fluid interface:
...
This intrinsic pressure difference creates the flow inside a fluid torus responsible for the shrinking instability in toroidal droplets, where the handle shrinks smoothly until it closes and the toroidal droplet becomes a spherical droplet~\cite{RN29}\fxnote{ref Alex}.
In addition, the Laplace pressure can also cause the tube of a toroidal droplet to break up reminiscent of the Rayleigh-Plateau instability such that the toroidal droplet breaks into one or more spherical droplets~\cite{RN29}\fxnote{ref Alex}.
While the study of the fluid modes in nontrivial topologies is interesting in its own right and has lead to interesting insights into fluid phenomena such as charged jets~\fxnote{ref Alex} and viscous fingering~\fxnote{ref Alex}, we are more concerned with creating a stable toroidal droplet. \\
...
To do so, we make our toroidal droplets using a yield-stress material as the outer phase~\cite{RN47}.
...
In addition, adding a pre-shear generaly helps the overall toroidal droplet be more axisymmetric.\\

We also find that

.. how to reduce climbing


.. how to load the active fluid with a dummy liquid

.. the specific material we use
