...
Once we have the re-estimated normal vectors everywhere on the surface, we can calculate the determinant of the metric~\cite{RN35},\fxnote{prove it?}
\begin{equation}
...
In addition, the majority of the pixels have $K$ within blah percent of the initial $K =1.1 \times 10^{-3}$ px$^{-2}$, signifying that we do not need to increase $d_2$ any further\fxnote{go to the distributions}.
...
\end{figure}\fxnote{add a saddle figure, briefly discuss outcomes without procedures}
...
Since $h(x,y)$ is a discrete quantity set by the height resolution of the confocal, the triangulated surface appears to have ``steps'' where the measured surface jumps from the height of one image plane to another.\fxnote{add schematic of the steps}
...
We remove these steps by downsampling the initial $h(x,y)$ until the output triangulation show no steps [see Figure~\fxnote{schematic}].
...
We then plot the relative root-mean-squared (RMS) fluctuations, $\displaystyle{\sqrt{\Delta N_{\Theta}^2}} \bigg / \displaystyle {\overbar{N}_{\Theta}}$, vs $\overbar{N}_{\Theta}$ for every region on all of our toroids on a log-log plot, as shown in Figure~\ref{f:3-ExpDistribution}(G).\fxnote{make sure I understand why you can do this}
...
From Eq.~\ref{e:3-NavierStokes} we know that $\alpha$ causes the nematic to deform.
...
Due to the large number of defects and their constant motion, the nematic dynamics become turbulent, with \fxnote{blah blah blah}~\cite{RN7}.
In this turbulent regime, we can write the characteristic velocity of the $s = +1/2$ defects as $v_0 \approx \alpha \ell_a/ \eta = \textrm{sign}(\alpha) \sqrt{|\alpha| k_F}/\eta$.\fxnote{introduce some balance for the velocities in the various regimes.}
...
In addition, the direction of the velocity of an $s = +1/2$ defect is determined by the defect's structure, while in passive nematics thermal fluctuations will cause the defect to diffuse isotropically.\fxnote{persistant random walkers?}
...
We calculate $\lambda'$ for each toroid.
Since the values for each ATP concentration are similar, we take a weighted average to obtain $\lambda' = 0.31 \pm 0.04$ and $\lambda' = 0.3 \pm 0.1$ for the 144 $\upmu$M and 36 $\upmu$M ATP concentrations, respectively.
...
We see that in our experiments, increasing $|\alpha|$ leads to increasing values of $\overbar{N}/A_{torus}$, consistent with the idea that $\ell_a = \sqrt{k_F / |\alpha|} \sim \lambda = \overbar{N}/A_{torus}$.
...
Rewriting the expression for $v'_0$ in terms of $\lambda'$, we have $v'_0 = (|\alpha|a^2 \lambda')/(\mu k_F \eta)$, implying that the large uncertainty in $v'_0$ for the 36 $\upmu$M ATP toroids comes from the large uncertainty in the associated $\lambda'$.
...
proxy for $\ell_a$.\fxnote{talk more about waht we see}
