...
Furthermore, in soft matter, defects have been used as tools to investigate a wide range of phenomena from knot theory~\cite{RN156}\fxnote{experimental knots} to conrolled self-assembly~\cite{RN43,RN50,RN150,RN157}, to heirarchical materials~\cite{RN164,RN159,RN27}.
...
\begin{figure}
  ...
  (C,D) Closed surfaces with a single hange like a (C) torus and a (D) coffee mug can be continuously deformed into each other.
  Howevewr, the surfaces in (A,B) cannot be continuously deformed in the surfaces in (C,D) as the process of adding a handle breaks the surface.}\label{f:1-ChiObjects}
\end{figure}
...
Since the individual particles in such an orientationally ordered phase prefer to align along each other, we can express the distortion free-energy in the continuum limit as a functional of the orientation field,
\begin{equation}
...
However, this configuration only exists in thin enough shells, as characterized by the relative shell thickness $h' = (R^{out}-R^{in})/R^{out}$, with $R^{out}$ the radius of the nematic shell and $R^{in}$ the radius of the inner water doplet, as defined schematically in Figure~\ref{f:1-Shells}(C).
...
\begin{figure}
  ...
  (C), Modulating the surface with a sine wave adds mean curvature but not Gaussian curavture to the surface; the homogeneous state on the surface is still possible.}\label{f:1-ParallelTransport}
\end{figure}
...
\begin{figure}
  ...
  (B), A normal section of a surface at a point corresponds to a planar curve, with the normal curvature at that point determined as in (A).
  (C), The principal curvatures $\kappa_1$ and $\kappa_2$ at a point are the maximum and minimum normal curvatures and are always in orthogonal directions.}\label{f:1-Curvature}
\end{figure}
