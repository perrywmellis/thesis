...
For any mask $\Theta(x,y)$ defining a region, we calculate the integrated Gaussian curvature numerically like $\oint_{\Theta}\,K\textrm{dA} \approx \sum_{x,y} \Theta(x,y) K(x,y) \sqrt{g}(x,y)$.
...
Thus, we can determine the number of $s = +1/2$ and $s = -1/2$ defects in a region denoted by $\Theta(x,y)$ over time $N^{(\pm)}_{\Theta}(t)$ with the sum $N^{(\pm)}_{\Theta}(t) = \sum_{xy}\,J^{(\pm)}(x,y)\Theta(x,y)$.
With $\overline{N}^{(\pm)}_{\Theta} = (1/T)\sum_t \, N^{(\pm)}_{\Theta}(t)$ we calculate the time-averaged topological charge in a region as $\overline{s}_{\Theta} = (\overline{N}^{(+)}_{\Theta} - \overline{N}^{(-)}_{\Theta})/2$.
...
We find that for both ATP concentrations, $\overline{s}_{\Theta}$ is linear with $\oint_{\Theta}\,K\textrm{dA}$, a shown in Fig.~\fxnote{Fig}, where we plot data from toroids with a range of $\xi$ and $a$.
The slope of the curve is positive, consistent with the curvature-induced defect unbinding predicted theoretically.
...
Furthermore, $\overline{s}_{\Theta}$ only depends on the integrated Gaussian curvature and is independent of $\xi$ and $a$.
...
We also note that the line in Fig.~\fxnote{Fig} goes through 0, indicating that regions with $\oint_{\Theta}\,K\textrm{dA} = 0$ have $\overline{s}_{\Theta} = 0$, a topological requirement for the entire toroid.
