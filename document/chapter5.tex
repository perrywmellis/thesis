...
Using results for 21 different bridges, we find an average $\Gamma_c = 2.7 \pm 0.3$, as shown in the upper contour in Figure~\xnote{Fig here}, where we have plotted each observation of a stable ring defect with open circles and of a stable point defect with ``x'' symbols.
The ring radius, scaled by the bridge height, varies linearly with $\Gamma$ for $\Gamma > \Gamma_c$, as indicated by the squares in Figure~\xnote{Fig here}, where we have again plotted every measurement we have performed.
At $\Gamma_c$, the  ring becomes unstable, and collapses to a point defect, yielding the discontinuity in $R_{ring}$ shown with a dashed line in Figure~\xnote{Fig here}, where the point defect is represented as having a vanishing $R_{ring}$.
...
As with the waist structures, we start at a large $\Gamma$ with the bridge containing a ring defect and decrease $\Gamma$ in discrete steps, monitoring the bridge over time at each step to ensure that the bridge has equilibrated.
As with the wasit-shaped bridges, we observe that the ring radius decreases with $\Gamma$.
However, in contrast to the waist-shaped bridges, we do not observe a transition from to a point defect in the barrel-shaped bridges.
