...
For example, consider the director structure of an $s = +1/2$ defect shown by the white lines in Figure~\ref{f:3-DefectFlow}(A); an extensile stress along the director will cause the bend distortion to grow and the splay distortion to decrease, causing defect to propagate along the red arrow.
...
We first determine the surface of the toroidal droplet by averaging each the image planes at a given height over time and then applying an intensity threshold.
...
\end{figure}\fxnote{make the lines in ACE thicker}
...
\subsection{Estimates of material parameters}\fxnote{remind what is alpha again.}
...
In addition, this result is consistent with the fact that when the ATP runs out in an experiment and the activity ceases, the nematic configuration becomes static and does not relax to the ground state; the elasticity in the experiments vanishes when $\alpha = 0$.\fxnote{this isn't right. what is the role of thermal fluctuations in the relaxation? connect to kT in active like in presentation. talk about active stress going in wrong direction. connect mobility with relaxation timescale.}
