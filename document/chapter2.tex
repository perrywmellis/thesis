...
While we briefly menthioned the dimensionality of defectsand their associated classifications schemes in INTRO\fxnote{ref Intro}, here we take a deeper look and use homeotopy theory as laid out in references~\cite{RN196,RN236,RN153} to introduce a more complete theory of defects in nematic materials.
As mentioned earlier in INTRO\fxnote{ref INTRO}, defects in ordered media are defined generally as regions where the characteristic local order is not satisfied.
...
Instead, for 2D nematics the order parameter space is $\mathbb{R}\mathbb{P}_1$, the real projective plane in 1D, which corresponds to $\mathbb{S}_1$ with antipodal points identified.
...
In the language of homeotopy thoery, we say that any two contours that can be continuously deformed into each other have a homeomorphism and thus are homeotopic to each other.
More generally, any contour in $\mathbb{R}\mathbb{P}_1$ with winding number $k$ is homeotopic to any other contour in $\mathbb{R}\mathbb{P}_1$ with winding number $k$, and only to contours with winding number $k$.
...
While we could form the group using the winding numbers in $\mathbb{R}\mathbb{P}_1$ themselves, it is more common to use the results from measuring the amount of director rotation in real space~\cite{RN23,RN153,RN203}.
Reproducing Eq.~[INTRO]\fxnote{ref INTRO} from [INTRO] for a director field parametrized by the angle $\phi(\mathbf{R})$, we characterize the defects with their ``defect charge'', $s$:
\begin{equation}
...
Thus, we have satisfied the four axioms for a group laid out in Table~\ref{t:2-GroupTheory}, such that we can characterize the homeotopy classes of defects in $\mathbb{R}\mathbb{P}_1$ with the first \emph{fundamental group} c.
Hence, the quantity $s$ is a topological quantity, such that ``defect charge'' is often referred to as ``topological charge.'' \\
...
In fact, for a general order parameter space $\mathbb{P}$ with dimension $t'$ and a defect with dimension $t$, the codimension $d = t'-t$ defines the order of the fundamental group needed to characterize the defects in $\mathbb{P}$, $\pi_{d}(\mathbb{P})$.
We note that for codimension $d=0$, there are no topological defects as the defect structure is homeotopic to a nonsingular distortion.
...
Immediately, we see that the order parameter space is no longer $\mathbb{R}\mathbb{P}_1$, but instead is $\mathbb{R}\mathbb{P}_2$, the unit 2-sphere $\mathbb{S}_2$ with antipodal points identified, as we now need 2 angles to characterize all the possible orientations of $\mathbf{n}$.
We first consider line defects with $d=1$ such that we characterize them with $\pi_{1}(\mathbb{R}\mathbb{P}_2)$.
Again, we consider $\mathbf{n}$ along closed contours in real space and the associated contour in $\mathbb{R}\mathbb{P}_2$.
...
Thus, the only nontrivial windings in $\mathbb{R}\mathbb{P}_2$ are those that run directly between antipodal points, hence $s \in \{-1/2,0,+1/2\}$.
...
Now instead of contours on $\mathbb{R}\mathbb{P}_2$, we consider coverings or wrappings of $\mathbb{R}\mathbb{P}_2$.
In real space, we calculate the topological ``hedgehog charge'' of a defect as:
\begin{equation}
...
The volume $V$ must be homeotopic to a sphere --- this means that the volume has neither holes nor handles such that the Euler characteristic is $\chi = 2$.
Physically, $q$ relates the orientations of $\mathbf{n}$ taken on a topologically spherical surface enclosing the defect to the number of times the orientations cover $\mathbb{S}_2$.
In addition, since there are 2 possible ways to project the apolar nematic orientation to the polar orientations on the unit 2-sphere, any structure in isolation can only be determined up to $|q|$ as the chosen projection will determine the sign.
