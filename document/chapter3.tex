...
\end{equation}
If the model is linear, the fit has an analytic solution~\fxnote{ref PRML}.
In addition, for normally-distributed data, the result of least squares fit gives the maximum-likelihood values of $\bm{\alpha}$~\fxnote{ref PRML}.
...
A weighted linear least-squares fit of this type is still analytically solvable~\fxnote{ref PRML}.
...
These cost functions are known as maximum-likelihood-type estimators, or M-estimators~\fxnote{ref PRML}.
...
A true least-squares fit has 100\% efficiency --- as data are included in the fit, a least-squared fit approaches the true values of the parameters the ``fastest,'' hence the fact that a least squares fit is 100\% efficient~\fxnote{ref PRML}.
