...
In 3D, defects are characterized by their ``hedgehog charge'' defined as $q = \frac{1}{4 \pi} \oint_{\partial V}d \theta d \phi \mathbf{n} \cdot \left [ \partial_{\theta} \mathbf{n} \times \partial_{\phi} \mathbf{n} \right ]$, where $\theta$ and $\phi$ are the polar and azimuthal spherical angles, respectively, and $\partial V$ is the bounding surface of the closed volume $V$ containing the defect.
...
Note that due to the added dimension, $q \in \mathbb{Z}$ --- half-integer hedgehog structures are unstable.
