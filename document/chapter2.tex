...
\begin{equation}
  f(\bm{\mathcal{Q}}) = f(S) = f_0 + \frac{1}{2}A S^2 + \frac{1}{3}B S^3 + \frac{1}{4}C S^4 + \mathcal{O} \left (S^5 \right ),\label{e:2-LdGTransGeneral}
\end{equation}
where $f(S) > 0$ implies the isotropic phase is stable against the nematic phase and $f(S) < 0$ corresponds to the nematic phase as the stable phase, with the phase transition occurring at $f(S) = 0$.
\begin{table}[t]
  \centering
...
Specifically, limiting the temperature dependence to only $A$ is also consistent with molecular theories of the nematic-isotroic phase transition.
Thus, let $A = A_0(T-T_{NI})$, $B = B_0$, and $C = C_0$, allowing us to rewrite Eq.~\ref{e:2-LdGTransGeneral} as:
\begin{equation}
  f(S) = f_0 + \frac{1}{2}A_0(T-T_{NI}) S^2 + \frac{1}{3}B_0 S^3 + \frac{1}{4}C_0 S^4.\label{e:2-LdGTransFinal}
\end{equation}
We see that Eq.~\ref{e:2-LdGTransFinal} predicts a first-order phase transition provided $B_0 \neq  0$, as a nonvanishing $B_0$ means that at $T = T_{NI}$, the minima in $f(S)$ occur at $S = 0$ and $S = -\frac{B}{C}$, giving a discontinuity in $S$ across the transtion.
...
\begin{equation}
  f(S) = f_0 + \frac{1}{2}A_0(T-T_{NI}) S^2 + \frac{1}{4}C_0 S^4.
\end{equation}
...
This can be seen by setting $T = T_{NI}$ and noticing that there is only one minimum in $f(S)$, and it occurs at $S=0$.
...
However, the nematic-isotropic phase transition has been explored heavily in simulations, and while the literature agree that the transition should be continuous, there is no clear consensus on the specific order of the transition.
