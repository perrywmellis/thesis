...
For general experiments with toroidal droplets where there is a large aamount of the inner phase available, we simply fill the syringe with the inner phase.
...
We save this cropped version in single multi-page \texttt{.tiff} file including the metadata concerning aquisition parameters.
...
Finally, we clear the raw confocal data from memory and we turn to determining the director and defects from our intensity projections.
...
We illustrate this technique with an example analysis of Figure~\fxnote{tings}.
...
The result of blurring the example full-stack image as well as the inset isolated defect pair in Figure~\fxnote{things} is seen in the main image and inset of Figure~\fxnote{things}, respectively.
...
We now define the coherence direction of a rank-2 tensor as the direction of the eigenvector associated with the smallest eigenvalue. The coherence direction represents the direction along which the spatial intensity fluctuations are the weakest and is periodic on the interval $[0^{\circ}, 180^{\circ})$.
The coherence direction calculated for the gradient tensor of each pixel for $I_\sigma$ shown in the main image and inset of Figure~\fxnote{things} is seen in the main image and inset of Figure~\fxnote{things}, where the orientation of the coherence direction on the interval $[0^{\circ}, 180^{\circ})$ has been mapped to the greyscale values $[\textrm{black}, \textrm{white})$ measured CW off of the horizontal axis.
We see that there are still too many fluctuations in the coherence directions shown in Figure~\fxnote{things} to determine $\mathbf{n}$.\\
...
The coherence direction of the structure tensor calculated with $\rho = 5$ px and filter size $29\textrm{ px } \times 29$ px for each pixel of $I_{\sigma}$ seen in the main image and inset of Figure~\fxnote{things} can be seen in the main image and inset in Figure~\fxnote{things}, respectively.
As before, the orientation on the interval $[0^{\circ}, 180^{\circ})$ has been mapped to the greyscale values $[\textrm{black}, \textrm{white})$ measured CW off of the horizontal axis.
...
From $\mathbf{u}$, we compute the 2D tensor nematic order parameter defined in Eq.~\fxnote{eq here}, taking the average over all the $\mathbf{u}$ of all points in a 5 pixel radius of the point of interest.
We then diagonalize the result as shown in Eq.~\fxnote{eq here}, providing $\mathbf{n}$ and $S$ for each pixel.
For the $\mathbf{u}$ in displayed in Figure~\fxnote{things}, $S$ is found in Figure~\fxnote{things}, where the greyscale intensity $[\textrm{black}, \textrm{gray})$ is mapped to the values $[0, 1]$ such that the dominant grey shade in Figure~\fxnote{things} indicates uniform alignment within the 5 px radius.
The corresponding $\mathbf{n}$ and defects are plotted in Figure~\fxnote{things}.
Note that only every $7^{\textrm{th}}$ value of $\mathbf{n}$ is plotted to ensure the $\mathbf{n}$-field is clear to the eye.
In reality there is a value for $\mathbf{n}$ at every pixel.


\subsection{Finding defect location and topological charge}
The defects are found by selecting pixels with $S < 0.1$ and then characterizing the $\mathbf{n}$-rotation about the point of interest.

he $+1/2$ and $-1/2$ defects are indicated in Fig. ED1f by ${\color{red} \blacktriangle } $  and  ${ \color{blue} \blacktriangledown } $, respectively.
